...
public class Note {
	...
	public void transpose(int steps) {
		...
		if(0<(noteIndex+steps) && (noteIndex+steps)<6){
			noteIndex += steps;
			//System.out.printf("Fall 1%n");
		} else if((noteIndex+steps)>6){
			noteIndex = steps-1; 
			//System.out.printf("Fall 2%n");
		} else if((noteIndex+steps) < 0){
			noteIndex = 7+noteIndex+steps;
			//System.out.printf("Fall 3%n");
		}	
	}
	...
	public String toString() {
		...
		//System.out.println(getNoteIndex());
		return String.format("%s %d",notes[getNoteIndex()], getBeats());
